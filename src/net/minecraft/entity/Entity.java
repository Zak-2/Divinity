...
public abstract class Entity implements ICommandSender {
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        // Find Me :: MoveFlyingInputEvent
        final MoveFlyingInputEvent inputEvent = new MoveFlyingInputEvent(forward, strafe, friction, rotationYaw);

        if (this instanceof EntityPlayerSP) ClientManager.getInstance().getEventDispatcher().post(inputEvent);
        if (inputEvent.isCancelled()) return;

        forward = inputEvent.getForward();
        strafe = inputEvent.getStrafe();
        friction = inputEvent.getFriction();
        ...
        if (f >= 1.0E-4F) {
            ...
            if (this instanceof EntityPlayerSP) yaw = inputEvent.getYaw();
