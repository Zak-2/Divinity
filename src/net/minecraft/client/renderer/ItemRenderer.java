...
import divinity.module.impl.render.ItemAnimations;
...
public class ItemRenderer {
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            if (this.itemToRender != null) {
                if (this.itemToRender.getItem() instanceof ItemMap) {
                ...
                } else if (abstractclientplayer.getItemInUseCount() > 0 || fakeUseTicks >= 0) {
                    ...
                    switch (enumaction) {
                        ...
                        case BLOCK:
                            if(ClientManager.getInstance().getModuleManager().get(ItemAnimations.class).isState()) {
                                doBlockAnimations(abstractclientplayer, partialTicks);
                            }else {
    ...
    private void doBlockAnimations(AbstractClientPlayer abstractClientPlayer, float partialTicks) {
        float f = 1.0F - (this.prevEquippedProgress + (this.equippedProgress - this.prevEquippedProgress) * partialTicks);
        float f1 = abstractClientPlayer.getSwingProgress(partialTicks);
        float swingProgress = MathHelper.sin(MathHelper.sqrt_float(f1) * 3.141592653F);
        float var16 = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * 3.1415927F);

        GlStateManager.translate(0.0F, 0.05F, 0.0F);
        switch (ClientManager.getInstance().getModuleManager().itemAnimations.blockModeProperty.getValue()) {
            case "OG":
    ...
    public void updateEquippedItem() {
        ...
        RenderItemStackEvent renderItemStackEvent = new RenderItemStackEvent(this.mc.thePlayer.inventory.getCurrentItem());
        ClientManager.getInstance().getEventDispatcher().post(renderItemStackEvent);

        EntityPlayer entityplayer = this.mc.thePlayer;
        ItemStack itemstack = renderItemStackEvent.stack;
        boolean flag = false;
        --fakeUseTicks;
