...
import divinity.gui.GuiClientMainMenu;
...
import net.minecraft.client.optifine.CustomColors;
import net.minecraft.client.optifine.GlErrors;
import net.minecraft.client.optifine.Lagometer;
import net.minecraft.client.optifine.RandomEntities;
import net.minecraft.client.optifine.gui.GuiChatOF;
import net.minecraft.client.optifine.reflect.Reflector;
import net.minecraft.client.optifine.reflect.ReflectorForge;
import net.minecraft.client.optifine.reflect.ReflectorResolver;
import net.minecraft.client.optifine.shaders.Shaders;
import net.minecraft.client.optifine.shaders.ShadersRender;
import net.minecraft.client.optifine.util.MemoryMonitor;
import net.minecraft.client.optifine.util.TextureUtils;
import net.minecraft.client.optifine.util.TimedEvent;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    // Todo :: Figure out Raytrace/Raycast
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    if (movingobjectposition != null) {
                        ...
                        if (d7 < d3 && !ClientManager.getInstance().getModuleManager().localRender.isNoWalls()) {
                            d3 = d7;
                        }
                    }
                }
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime) {
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (this.mc.theWorld != null) {
                ...
                this.renderWorld(scaledresolution, partialTicks, System.nanoTime() + l);
    ...
    private void renderWorldPass(ScaledResolution scaledResolution, int pass, float partialTicks, long finishTimeNano) {
        ...
        ClientManager.getInstance().getEventDispatcher().post(new RenderWorldEvent(partialTicks, scaledResolution));
    ...
    private void addRainParticles() {
        if (ClientManager.getInstance().getModuleManager().atmosphere.forceSnow()) {
            return;
        }
    ...
    protected void renderRainSnow(float partialTicks) {
        ...
        boolean forceSnow = ClientManager.getInstance().getModuleManager().atmosphere.forceSnow();
                ...
                for (int l1 = i - i1; l1 <= i + i1; ++l1) {
                    ...
                    if (biomegenbase.canRain() || biomegenbase.getEnableSnow()) {
                        ...
                        if (k2 != l2) {
                            ...
                            if (!forceSnow && world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15F) {
                                if (j1 != 0) {
                                    if (j1 >= 0) {
                                        tessellator.draw();
                                    }
    ...
    private void setupFog(int startCoords, float partialTicks) {
        ...
        if (ClientManager.getInstance().getModuleManager().atmosphere.overrideFog()) {
            if (block.getMaterial() != Material.water && block.getMaterial() != Material.lava && !this.cloudFog) {
                if (ClientManager.getInstance().getModuleManager().atmosphere.fogNone()) {
                ...
                if (ClientManager.getInstance().getModuleManager().atmosphere.fogClear()) {
                    float f3Clear = this.farPlaneDistance * 4.0F;
                    GlStateManager.setFogStart(0.0F);
                    GlStateManager.setFogEnd(f3Clear);
                } else if (ClientManager.getInstance().getModuleManager().atmosphere.fogOverride()) {
                    float start = ClientManager.getInstance().getModuleManager().atmosphere.getFogStartDistance();
                    float end = ClientManager.getInstance().getModuleManager().atmosphere.getFogEndDistance();
