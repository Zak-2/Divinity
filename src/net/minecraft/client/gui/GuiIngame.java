...
import divinity.gui.chat.GuiDivinityChat;
import divinity.module.impl.render.Crosshair;
...
public class GuiIngame extends Gui {
    ...
    private final GuiDivinityChat divinityChat;
    ...
    public GuiIngame(Minecraft mcIn) {
        ...
        this.divinityChat = new GuiDivinityChat(mcIn);
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        // Find ME :: RenderGuiEvent
        ClientManager.getInstance().getEventDispatcher().post(new RenderGuiEvent(scaledresolution, partialTicks));
        GlStateManager.resetColor();

        // Find ME :: Notifications Render
        ClientManager.getInstance().getNotificationManager().render();

        // FIND ME :: Draw blur overlay
        BlurUtil.onRenderGameOverlay(this.mc.getFramebuffer(), scaledresolution);

        BloomUtil.onRenderGameOverlay(this.mc.getFramebuffer());
        ...
        if (this.showCrosshair() && !ClientManager.getInstance().getModuleManager().get(Crosshair.class).isState()) {
        ...
        if (ClientManager.getInstance().isMcChat) this.persistantChatGUI.drawChat(this.updateCounter);
        else this.divinityChat.drawChat(this.updateCounter);
        GlStateManager.popMatrix();
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        int arrayHeight = ((HUD) ClientManager.getInstance().getModuleManager().get(HUD.class)).totalHeight;
        int i1 = collection.size() * this.getFontRenderer().FONT_HEIGHT;
        int j1old = scaledRes.getScaledHeight() / 2 + i1 / 3;
        int j1 = Math.max(arrayHeight + i1 + 12, j1old);
        int k1 = 3;
    ...
    public GuiDivinityChat getDivinityChatGUI() {
        return this.divinityChat;
    }
}
