...
import divinity.utils.ChatColorUtils;
...
public class GuiNewChat extends Gui {
    ...
    public void printChatMessageWithOptionalDeletion(IChatComponent chatComponent, int chatLineId) {
        String originalMessage = chatComponent.getUnformattedText();

        ClientManager.getInstance().getEventDispatcher().post(new ChatEvent(originalMessage));
        this.setChatLine(chatComponent, chatLineId, this.mc.ingameGUI.getUpdateCounter(), false);

        logger.info("[CHAT] " + originalMessage);
    }


    private EnumChatFormatting getEnumChatFormattingFromCode(String colorCode) {
        if (colorCode == null || colorCode.length() < 2) return EnumChatFormatting.WHITE;

        char prefix = colorCode.charAt(0);
        char codeChar;
        if (prefix == '§' || prefix == '\u00A7' || prefix == '&') {
            codeChar = colorCode.charAt(1);
        } else {
            codeChar = colorCode.charAt(0);
        }
