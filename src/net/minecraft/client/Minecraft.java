...
public class Minecraft implements IThreadListener {
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        ClientManager.getInstance().moduleManager = new ModuleManager();
    ...
    private void runGameLoop() throws IOException {
        ...
        final int deltaTime = (int) (currentTime - ClientManager.getInstance().lastFrame);
        ClientManager.getInstance().lastFrame = currentTime;
        ClientManager.getInstance().delta = deltaTime;

        // Find Me :: GameTickEvent (Fairly Sure this should be hooked here)
        ClientManager.getInstance().getEventDispatcher().post(new GameTickEvent());
        ...
        tickCounter++;
        int x12a = tickCounter;

        if (!this.skipRenderWorld) this.entityRenderer.updateCameraAndRender(this.timer.renderPartialTicks, i);

        if (this.gameSettings.showDebugInfo && this.gameSettings.showDebugProfilerChart && !this.gameSettings.hideGUI) {
            // Profiler Debugging would be here.
        } else {
        ...
        if (this.isFramerateLimitBelowMax()) Display.sync(this.getLimitFramerate());
    ...
    public void runTick() throws IOException {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            while (Mouse.next()) {
                ...
                ClientManager.getInstance().getEventDispatcher().post(new MouseClickEvent(i, Mouse.getEventButtonState()));
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    ...
                    if (this.currentScreen != null) {
                        ...
                        if(k == Keyboard.KEY_INSERT){
                            displayGuiScreen(ClientManager.getInstance().getGuiEditElement());
                        }
                        for (Module module : ClientManager.getInstance().getModuleManager().getModules()) {
                            if (k == module.getKeyBind()) module.toggle();
                        }

                        TabGui.getInstance().keyListener(k);
            ...
            // Find Me :: HandleInputEvent
            ClientManager.getInstance().getEventDispatcher().post(new HandleInputEvent());
    ...
    public void loadWorld(WorldClient worldClientIn, String loadingMessage) {
        ...
        ClientManager.getInstance().getEventDispatcher().post(new LoadWorldEvent());
        System.gc();
        this.systemTime = 0L;
    }
