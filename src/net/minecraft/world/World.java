...
public abstract class World implements IBlockAccess {
    ...
    public Vec3 getSkyColor(Entity entityIn, float partialTicks) {
        if (ClientManager.getInstance().getModuleManager().atmosphere.fogOverride()) {
            return ClientManager.getInstance().getModuleManager().atmosphere.getOverrideSkyColor();
        }
    ...
    public Vec3 getFogColor(float partialTicks) {
        if (ClientManager.getInstance().getModuleManager().atmosphere.fogOverride()) {
            return ClientManager.getInstance().getModuleManager().atmosphere.getOverrideFogColor();
        }
        float f = this.getCelestialAngle(partialTicks);
    ...
    public float getThunderStrength(float delta) {
        float forced = ClientManager.getInstance().getModuleManager().atmosphere.getThunderStrengthOverride(delta);
        if (forced >= 0.0F) {
            return forced;
        }
    ...
    public float getRainStrength(float delta) {
        float forced = ClientManager.getInstance().getModuleManager().atmosphere.getRainStrengthOverride(delta);
        if (forced >= 0.0F) {
            return forced;
        }
