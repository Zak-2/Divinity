...
public class FontRenderer {
    ...
    protected final int drawChar(char chr, double x, double y) {
        int region = chr >> 8;
        int id = chr & 0xFF;
        int xTexCoord = (id & 0xF) * fontWidth,
                yTexCoord = (id >> 4) * fontHeight;
        ...
        glVertex2d(x, y);
        glTexCoord2d(wrapTextureCoord(xTexCoord, textureWidth), wrapTextureCoord(yTexCoord + fontHeight, textureHeight));
        glVertex2d(x, y + fontHeight);
        glTexCoord2d(wrapTextureCoord(xTexCoord + width, textureWidth), wrapTextureCoord(yTexCoord + fontHeight, textureHeight));
        glVertex2d(x + width, y + fontHeight);
        glTexCoord2d(wrapTextureCoord(xTexCoord + width, textureWidth), wrapTextureCoord(yTexCoord, textureHeight));
    ...
    public void drawCenteredStringGradient(String str, float x, float y, int startColor, int endColor) {
        drawStringGradient(str, x - getStringWidth(str) / 2, y, startColor, endColor);
    }

    public void drawStringGradient(String str, double x, double y, int startColor, int endColor) {
        GlStateManager.color(1F, 1F, 1F, 1F);
        str = str.replace("▬", "=");
        y = y - 2;
        x *= 2;
        ...
        for (int i = 0; i < length; i++) {
            ...
            // Handle color formatting codes
            if (chr == '\u00A7' && i != length - 1) {
                i++;
                int colorIndex = "0123456789abcdef".indexOf(chars[i]);
                if (colorIndex != -1) {
                    int colorCodeValue = colorCode[colorIndex];
//                    if (darken) colorCodeValue = (colorCodeValue & 0xFCFCFC) >> 2 | colorCodeValue & 0xFF000000;

                    float r = (colorCodeValue >> 16 & 0xFF) / 255f;
                    float g = (colorCodeValue >> 8 & 0xFF) / 255f;
                    float b = (colorCodeValue & 0xFF) / 255f;
                    float a = (colorCodeValue >> 24 & 0xFF) / 255f;
