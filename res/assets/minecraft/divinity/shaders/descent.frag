...
vec3 addSign(vec3 color, vec3 pos, float side, vec2 id) {
    ...
    float charFrag = -3.5 + 8.0 * noise(id + 6.0 * charPos);

    // Normalize position within the cell
    charPos = fract(charPos);

    // Apply horizontal and vertical smooth masking
    charFrag *= smoothstep(0.0, 0.4, charPos.x) * smoothstep(1.0, 0.6, charPos.x);
    charFrag *= smoothstep(0.0, 0.4, charPos.y) * smoothstep(1.0, 0.6, charPos.y);

    // Blend signColor into base color using conditional mask
    color = mix(color, signColor, flash * flicker * step(outline, 0.01) * clamp(charFrag, 0.0, 1.0));
...
void main() {
    ...
    color += addLight(eye, ray, res.x, time, lightHeight - 0.6);
